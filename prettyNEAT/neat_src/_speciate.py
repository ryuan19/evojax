...
def assignSpecies(self, species, pop, p):
  ...
  for i in range(len(pop)):
    assigned = False
    for iSpec in range(len(species)):
      ref = jnp.array(species[iSpec].seed.conn)
      ind = jnp.array(pop[i].conn)
...
def assignOffspring(self, species, pop, p):
  ...
  if nSpecies == 1:
    ...
    popFit = jnp.array([ind.fitness for ind in pop])
    ...
    if p['select_rankWeight'] == 'exp':
      ...
      rankScore = 1+jnp.abs(popRank - len(popRank))
    specId = jnp.array([ind.species for ind in pop])
    ...
    for iSpec in range(nSpecies):
      if not jnp.any(specId == iSpec):
        ...
        speciesFit = speciesFit.at[iSpec].set(jnp.mean(rankScore[specId == iSpec]))
        speciesTop = speciesTop.at[iSpec].set(jnp.max(popFit[specId == iSpec]))
...
def compatDist(self, ref, ind):
  ...
  ind = ind.at[3, :].set(jnp.where(jnp.isnan(ind[3, :]), 0, ind[3, :]))
  ref = ref.at[3, :].set(jnp.where(jnp.isnan(ref[3, :]), 0, ref[3, :]))
  weightDiff = jnp.abs(ind[3, IA] - ref[3, IB])
  geneDiff = jnp.sum(~IA) + jnp.sum(~IB)
...
def unregister(aux_data, flattened):
    n = Species(flattened[0])
    n.members = flattened[1]
    n.bestInd = flattened[2]
    n.bestFit = flattened[3]
    n.lastImp = flattened[4]
    n.nOffspring = flattened[5]
    return n


jax.tree_util.register_pytree_node(Species, register, unregister)
